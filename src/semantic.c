...
void validate_node(ASTNode *node) {
  ...
  case NODE_IDENTIFIER: {
    ...
    if (id == -1) {
      semantic_error(node->t.line, "Variable no declarada o fuera de ámbito:",
                     node->t.lexeme);
  ...
  case NODE_LITERAL: {
    ...
    if (node->t.type == TKN_LIT_INT)
      node->eval_type = TKN_INT;
    else if (node->t.type == TKN_LIT_FLOAT)
      node->eval_type = TKN_FLOAT;
    else if (node->t.type == TKN_LIT_STRING)
      node->eval_type = TKN_STRING;
    else if (node->t.type == TKN_LIT_TRUE || node->t.type == TKN_LIT_FALSE)
  ...
  case NODE_UNARY_OP: {
    ...
    if (node->t.type == TKN_NOT) {
      ...
      if (operand_type != TKN_BOOL && operand_type != TKN_ERROR) {
        semantic_error(node->t.line,
                       "Operación unaria lógica ! sobre tipo no booleano",
                       node->t.lexeme);
        node->eval_type = TKN_ERROR;
      } else {
        node->eval_type = TKN_BOOL;
      ...
      break;
    } else if (node->t.type == TKN_SELF_PLUS ||
               node->t.type == TKN_SELF_MINUS) {

      if (node->t.type == TKN_SELF_MINUS && operand_type == TKN_STRING) {
        semantic_error(
            node->t.line,
            "Operación -- no está soportada para tipo string:", node->t.lexeme);
        node->eval_type = TKN_ERROR;
      } else if (operand_type == TKN_BOOL) {
        semantic_error(node->t.line,
                       "Operación unaria ++/-- no soportada para tipo bool:",
                       node->t.lexeme);
        node->eval_type = TKN_ERROR;
        ...
        semantic_error(
            node->t.line,
            "Operación unaria iterativa ++/-- sobre tipo no numérico.",
            node->t.lexeme);
        node->eval_type = TKN_ERROR;
      } else {
  ...
  case NODE_BINARY_OP: {
    ...
    if (node->t.type == TKN_AND || node->t.type == TKN_OR) {
      ...
      if (leftType == TKN_BOOL && rightType == TKN_BOOL) {
        ...
        semantic_error(node->t.line, "Tipos incompatibles en operación lógica:",
                       node->t.lexeme);
        node->eval_type = TKN_ERROR;
      }
    } else if (node->t.type == TKN_PLUS || node->t.type == TKN_MINUS ||
               node->t.type == TKN_MULT || node->t.type == TKN_DIV ||
               node->t.type == TKN_POWER) {
      ...
      if (leftType == TKN_INT && rightType == TKN_INT) {
      ...
      } else if (node->t.type == TKN_PLUS && leftType == TKN_STRING &&
                 rightType == TKN_STRING) {
        ...
        semantic_error(
            node->t.line,
            "Operación aritmética no válida con tipo string:", node->t.lexeme);
        node->eval_type = TKN_ERROR;
        ...
        semantic_error(
            node->t.line,
            "Operación aritmética no válida con tipo bool:", node->t.lexeme);
        node->eval_type = TKN_ERROR;
        ...
        semantic_error(
            node->t.line,
            "Tipos incompatibles en operación aritmética:", node->t.lexeme);
        node->eval_type = TKN_ERROR;
      }
    } else if (node->t.type == TKN_GREATER || node->t.type == TKN_LESS ||
               node->t.type == TKN_GREATER_EQUAL ||
               node->t.type == TKN_LESS_EQUAL || node->t.type == TKN_EQUAL ||
               node->t.type == TKN_NOT_EQUAL) {

      int left_ok = (leftType == TKN_INT || leftType == TKN_FLOAT ||
                     leftType == TKN_BOOL || leftType == TKN_STRING);
      int right_ok = (rightType == TKN_INT || rightType == TKN_FLOAT ||
                      rightType == TKN_BOOL || rightType == TKN_STRING);
      ...
      int relational_only =
          (node->t.type == TKN_GREATER || node->t.type == TKN_LESS ||
           node->t.type == TKN_GREATER_EQUAL || node->t.type == TKN_LESS_EQUAL);

      if (!left_ok || !right_ok || !compatible) {
        semantic_error(node->t.line,
                       "Tipos incompatibles en comparación:", node->t.lexeme);
        node->eval_type = TKN_ERROR;
      } else if (relational_only &&
                 (leftType == TKN_BOOL || leftType == TKN_STRING)) {
        semantic_error(node->t.line,
                       "Operador relacional (<, >, <=, >=) no soportado para "
                       "tipo bool o string:",
                       node->t.lexeme);
        node->eval_type = TKN_ERROR;
      } else {
  ...
  case NODE_ASSIGN: {
    ...
    if (target_type != expr_type) {
      ...
      if (!(target_type == TKN_FLOAT && expr_type == TKN_INT)) {
        ...
        semantic_error(node->t.line, "Tipos incompatibles en la asignación.",
                       NULL);
        node->eval_type = TKN_ERROR;
      }
    }
  ...
  case NODE_READ: {
    if (!node->left || node->left->type != NODE_IDENTIFIER) {
      semantic_error(node->t.line, "Operación read requiere un identificador",
                     NULL);
      node->eval_type = TKN_ERROR;
    } else {
